...
import { IdleTokenAnimationMotionSettingsApp } from "./settingsApp.js";
 ...
 * a compact popup.
 *
 * Eligibility and HP settings remain normal Foundry settings.
 */
export function registerSettings() {
  game.settings.registerMenu(MODULE_ID, "motionSettings", {
  ...
  game.settings.register(MODULE_ID, "amount", {
  ...
  game.settings.register(MODULE_ID, "bobPx", {
    name: "Vertical Bob",
  ...
  game.settings.register(MODULE_ID, "swayPx", {
  ...
  game.settings.register(MODULE_ID, "rollDeg", {
    name: "Tilt Angle",
  ...
  game.settings.register(MODULE_ID, "freqHz", {
    name: "Motion Speed",
  ...
  game.settings.register(MODULE_ID, "noise", {
    name: "Irregularity",
  ...
  game.settings.register(MODULE_ID, "randomPhase", {
  ...
  game.settings.register(MODULE_ID, "respectHidden", {
  ...
  game.settings.register(MODULE_ID, "respectCombat", {
  ...
  game.settings.register(MODULE_ID, "filterZeroHpTokens", {
    name: "Filter 0 HP Tokens",
    hint: "Only animate tokens whose actor HP resolves above 0. Disable this to ignore HP entirely.",
    scope: "world",
    config: true,
    type: Boolean,
    default: DEFAULT_SETTINGS.filterZeroHpTokens,
    restricted: true,
    onChange: requestSync
  });

  game.settings.register(MODULE_ID, "hpDetectionMode", {
    ...
    config: true,
    type: String,
    default: DEFAULT_SETTINGS.hpDetectionMode,
    choices: {
      auto: "Automatic",
      custom: "Custom Path"
    },
  ...
  game.settings.register(MODULE_ID, "customHpPath", {
    name: "Custom HP Path",
    ...
    config: true,
    type: String,
    default: DEFAULT_SETTINGS.customHpPath,
    restricted: true,
    onChange: requestSync
  });

  game.settings.register(MODULE_ID, "defaultActorOptOut", {
    name: "Default Actor Opt-Out",
    hint: "Treat actors as opted out unless explicitly enabled later. Leave off for normal behaviour.",
    scope: "world",
    config: true,
    type: Boolean,
    default: DEFAULT_SETTINGS.defaultActorOptOut,
    restricted: true,
    onChange: requestSync
  });
}
...
export function getFilterZeroHpTokens() {
  return game.settings.get(MODULE_ID, "filterZeroHpTokens") !== false;
}
...
export function buildAnimationParamsFromSettings() {
  const amount = clamp(
    Number(game.settings.get(MODULE_ID, "amount")),
    0,
    5
  );

  const bobPx = clamp(
    Number(game.settings.get(MODULE_ID, "bobPx")),
    0,
    10
  );

  const swayPx = clamp(
    Number(game.settings.get(MODULE_ID, "swayPx")),
    0,
    10
  );

  const rollDeg = clamp(
    Number(game.settings.get(MODULE_ID, "rollDeg")),
    0,
    15
  );

  const freqHz = clamp(
    Number(game.settings.get(MODULE_ID, "freqHz")),
    0.01,
    3
  );

  const noise = clamp(
    Number(game.settings.get(MODULE_ID, "noise")),
    0,
    0.5
  );

  const randomPhase = Boolean(game.settings.get(MODULE_ID, "randomPhase"));
